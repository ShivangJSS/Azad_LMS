...
def _to_minutes(value: Optional[str]) -> Optional[int]:
    """
    module_duration is a free-text column.

    Only plain numeric values are treated as minutes.
    Values such as '45 Days' return None.
    """
...
class MobileModuleService:
    ...
    def list_modules(
        ...
        # IMPORTANT:
        #
        # Repository already returns modules in sequence_no order.
        #
        # Only the FIRST incomplete module can be ACTIVE.
        # Every later incomplete module must remain LOCKED.
        # ---------------------------------------------------------
        ...
        for row in rows:
            ...
            # -----------------------------------------------------
            # Completed modules keep their COMPLETED status.
            # -----------------------------------------------------
            if row.lock_status == LOCK_STATUS_COMPLETED:
                module_lock_status = LOCK_STATUS_COMPLETED
                completed += 1

            # -----------------------------------------------------
            # First incomplete module becomes ACTIVE.
            # -----------------------------------------------------
            elif not active_assigned:
                module_lock_status = LOCK_STATUS_ACTIVE
                active_assigned = True

            # -----------------------------------------------------
            # Every incomplete module after the first one is LOCKED.
            # -----------------------------------------------------
            else:
                module_lock_status = LOCK_STATUS_LOCKED
            ...
            modules.append(
                ModuleListItem(
                    ...
                    language_id=row.language_id,
                    status=module_status,
                    is_locked=module_status == "locked",
                )
            )
