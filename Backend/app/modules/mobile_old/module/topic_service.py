...
def _media_url(path: Optional[str], base_url: Optional[str] = None) -> Optional[str]:
    if not path:
        return None

    clean_path = path.replace("\\", "/").lstrip("/")
    relative_url = f"/mobile/module/media/{clean_path}"
...
class TopicService:
    ...
    def list_topics(
        ...
        for row in rows:
            ...
            is_uploaded_video = (
                row.doc_type or ""
            ).strip().lower() == "video" and not row.youtube_url
            topics.append(
                TopicItem(
                    ...
                    media_url=_media_url(content_path, media_base_url),
                    video_url=(
                        _media_url(content_path, media_base_url)
