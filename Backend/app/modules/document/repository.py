...
class DocumentRepository:
    ...
    def get_pdf(db: Session, pdf_id: int):
        return db.query(PdfMaster).filter(PdfMaster.pdf_id == pdf_id).first()

    @staticmethod
    def get_ppt(db: Session, ppt_id: int):
        return db.query(PptMaster).filter(PptMaster.ppt_id == ppt_id).first()

    @staticmethod
    def get_video(db: Session, video_id: int):
        return db.query(VideoMaster).filter(VideoMaster.video_id == video_id).first()
    ...
    def delete_document(
        ...
        document.deleted_at = datetime.utcnow()  # type: ignore
