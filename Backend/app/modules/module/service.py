...
def _assert_module_name_available(
    db, fk_course_id, module_name, language_id=1, exclude_id=None
):
    """Reject a duplicate module name within the same course AND language.

    Scoped per language so an independent module added on the Hindi tab,
    say, never collides with an unrelated English module of the same name —
    language data is completely independent.
    """
...
class ModuleService:
    ...
    def create_module(
        ...
        _assert_module_name_available(
            db, fk_course_id, module_name, language_id=language_id
        )
    ...
    def update_module(
        ...
        current = (
            db.query(ModuleMaster.language_id)
            .filter(
                ModuleMaster.module_id == module_id, ModuleMaster.deleted_at.is_(None)
            )
            .scalar()
        )
        _assert_module_name_available(
    ...
    def get_post_assessments(
        ...
        assessments = ModuleRepository.get_post_assessments_by_module(
        ...
        return ModulePostAssessmentResponse(
            ...
            assessments=[
                AssessmentResponse(
                    assessment_id=row.assessment_id, assessment_name=row.assessment_name
                )
    ...
    def get_pre_assessments(
        ...
        assessments = ModuleRepository.get_pre_assessments_by_module(
            db=db, module_id=module_id
        )
    ...
    @staticmethod
    def _mapped_group_langs(rows, id_attr):
        """group_id -> set of language_ids that have a DIRECTLY-mapped question.

        A group id is the row's parent_id (or its own id when it is the base
        question). Lets a single stored mapping resolve across a question's
        language variants without duplicating the mapping per language.
        """
        groups = {}
        for row in rows:
            if row.is_checked:
                grp = row.parent_id or getattr(row, id_attr)
                groups.setdefault(grp, set()).add(row.language_id)
        return groups

    @staticmethod
    def _is_checked_across_langs(row, id_attr, mapped_group_langs):
        """Checked if this row is the mapped question OR the translation of one.

        Exact self-match handles the assigned language (and avoids ticking a
        same-language sibling in the same parent group); a mapped question in
        the group under a DIFFERENT language means this row is its translation.
        """
        if row.is_checked:
            return True
        grp = row.parent_id or getattr(row, id_attr)
        langs = mapped_group_langs.get(grp)
        return bool(langs) and any(lang != row.language_id for lang in langs)

    @staticmethod
    def get_scq_assessment_mapping(
        db: Session, assessment_id: int, language_id: int | None = None
    ):
        rows = ModuleRepository.get_scq_assessment_mapping(
            db=db, assessment_id=assessment_id
        )

        # The assignment is stored once (single mapping, no per-language
        # duplication). A question is checked when EITHER its own id is mapped
        # (exact — so a same-language sibling in the parent group is not falsely
        # ticked) OR a mapped question in its parent group belongs to a DIFFERENT
        # language (i.e. this row is the translation of an assigned question).
        mapped_group_langs = ModuleService._mapped_group_langs(rows, "scq_id")
        ...
        return [
            {
                ...
                "is_checked": ModuleService._is_checked_across_langs(
                    row, "scq_id", mapped_group_langs
                ),
            }
            for row in rows
    ...
    def get_mcq_assessment_mapping(
        ...
        # Single shared mapping resolved across languages (see
        # get_scq_assessment_mapping for the rule).
        mapped_group_langs = ModuleService._mapped_group_langs(rows, "mcq_id")
    ...
    def get_match_making_assessment_mapping(
        ...
        records = ModuleRepository.get_match_making_assessment_mapping(
            db=db, assessment_id=assessment_id
        )

        # Single shared mapping resolved across languages (see
        # get_scq_assessment_mapping for the rule).
        mapped_group_langs = ModuleService._mapped_group_langs(
            records, "match_making_id"
        )

        return [
            {
                "match_making_id": row.match_making_id,
                "match_making_question_title": row.match_making_question_title,
                "match_making_question_description": row.match_making_question_description,
                "image_url": row.image_url,
                "status": row.status,
                "marks": float(row.marks) if row.marks is not None else None,
                "language_id": row.language_id,
                "language_name": row.language_name,
                "is_checked": ModuleService._is_checked_across_langs(
                    row, "match_making_id", mapped_group_langs
                ),
            }
            for row in records
            if language_id is None or row.language_id == language_id
    ...
    def get_drop_bucket_assessment_mapping(
        ...
        records = ModuleRepository.get_drop_bucket_assessment_mapping(
            db=db, assessment_id=assessment_id
        )
        ...
        # get_scq_assessment_mapping for the rule).
        mapped_group_langs = ModuleService._mapped_group_langs(
            records, "drop_bucket_id"
        )

        return [
            {
                "drop_bucket_id": row.drop_bucket_id,
                "drop_bucket_question_title": row.drop_bucket_question_title,
                "drop_bucket_question_description": row.drop_bucket_question_description,
                "image_url": row.image_url,
                "status": row.status,
                "marks": float(row.marks) if row.marks is not None else None,
                "language_id": row.language_id,
                "language_name": row.language_name,
                "is_checked": ModuleService._is_checked_across_langs(
                    row, "drop_bucket_id", mapped_group_langs
                ),
            }
            for row in records
            if language_id is None or row.language_id == language_id
    ...
    @staticmethod
    def save_assessment_mapping(db, request):
        count = ModuleRepository.save_assessment_mapping(
            db=db,
    ...
    def get_topics_for_main_content(
        ...
        return ModuleRepository.get_topics_for_main_content(
            db=db,
            module_id=module_id,
        )
    ...
    def get_documents_for_main_content(
        ...
        return ModuleRepository.get_documents_for_main_content(
            db=db,
        )
    ...
    def create_main_content(
        ...
        exists = ModuleRepository.check_main_content_exists(
            db=db,
        ...
        try:
            ...
            return {"success": True, "message": "Content assigned successfully"}
    ...
    def get_main_content_list(
        ...
        return ModuleRepository.get_main_content_list(
            db=db,
            module_id=module_id,
            language_id=language_id,
            listing_type=listing_type,
