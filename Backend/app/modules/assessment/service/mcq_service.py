...
class McqService:
    ...
    # If translation doesn't exist, return English
     if not mcq:
        english_mcq = McqRepository.get_by_parent_and_language(
            db=db,
            parent_id=parent_id,
            language_id=1,
        )
        ...
        english_mcq.options = McqRepository.get_options(
            db,
            english_mcq.mcq_id,
        )
    ...
    def validate_mcq(data: McqCreate):
        ...
        if not data.mcq_question_title.strip():
            raise HTTPException(
                status_code=400,
                detail="Question title is required."
            )
    ...
    def validate_options(options):
        ...
        if len(options) < 2:
            raise HTTPException(
                status_code=400,
                detail="Minimum 2 options are required."
            )

        if len(options) > 6:
            raise HTTPException(
                status_code=400,
                detail="Maximum 6 options are allowed."
            )

        correct = sum(
            1
            for option in options
            if option.is_mcq_option_correct == 1
        )
        ...
        for option in options:
            ...
            if not option.mcq_option_text.strip():
                raise HTTPException(
                    status_code=400,
                    detail="Option text cannot be empty."
                )
    ...
    def export_mcqs(
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": "attachment; filename=mcq_list.xlsx"
            },
        )
