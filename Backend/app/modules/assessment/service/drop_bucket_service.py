...
class DropBucketService:
    ...
    def get_by_id(
        ...
        if language_id == 1:
            ...
            drop_bucket.is_translation = (
                False  # Fixed: Base English is not a translation
            )
        ...
        # Fallback: Translation doesn't exist, return blank state
    ...
    def _language_names(db: Session) -> dict:
        return {
            row.language_id: row.language_name for row in db.query(LanguageMaster).all()
        }
    ...
    def get_all_languages(db: Session, parent_id: int):
        english = DropBucketRepository.get_by_id(db=db, drop_bucket_id=parent_id)
        if not english:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Drop Bucket not found.",
        ...
        languages = [
            {
                ...
                "language_name": lang_names.get(english.language_id) or "English",
            }
        ]

        # Fetch English Buckets strictly
        ...
        # Fetch Translation Buckets strictly per translation
        for tr in DropBucketRepository.get_translations(db=db, parent_id=parent_id):
            languages.append(
                {
    ...
    def bulk_update(db: Session, parent_id: int, data):
        ...
        english.drop_bucket_question_description = data.drop_bucket_question_description
    ...
    def get_question_items(
        ...
        master = DropBucketRepository.get_by_id(db=db, drop_bucket_id=drop_bucket_id)
        if not master:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Drop Bucket not found.",
    ...
    def update_bucket_items(
        ...
        bucket = (
            db.query(DropBucket)
            .filter(DropBucket.bucket_id == bucket_id, DropBucket.deleted_at.is_(None))
            .first()
        )
