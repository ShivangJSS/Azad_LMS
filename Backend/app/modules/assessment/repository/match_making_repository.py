...
from app.modules.assessment.model import MatchLeftItem, MatchMakingMaster, MatchRightItem
...
class MatchMakingRepository:
    ...
    def get_all(
        ...
        if search:
            query = query.filter(
                MatchMakingMaster.match_making_question_title.ilike(
                    f"%{search}%"
                )
            )

        return query.order_by(
            MatchMakingMaster.match_making_id.desc()
        ).all()
    ...
    def get_left_items(
     ...
     if language_id is not None:
        query = query.filter(
            MatchLeftItem.language_id == language_id
        )

     return query.order_by(
        MatchLeftItem.match_left_id.asc()
    ).all()
    ...
    def get_right_items(
     ...
     if language_id is not None:
        query = query.filter(
            MatchRightItem.language_id == language_id
        )

     return query.order_by(
        MatchRightItem.match_right_id.asc()
    ).all()
